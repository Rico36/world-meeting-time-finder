const cities = [
  ['New York','United States','America/New_York','US','US-NY'],['Los Angeles','United States','America/Los_Angeles','US','US-CA'],['Chicago','United States','America/Chicago','US','US-IL'],['Toronto','Canada','America/Toronto','CA','CA-ON'],['Vancouver','Canada','America/Vancouver','CA','CA-BC'],['Mexico City','Mexico','America/Mexico_City','MX','MX-CMX'],['São Paulo','Brazil','America/Sao_Paulo','BR','BR-SP'],['Buenos Aires','Argentina','America/Argentina/Buenos_Aires','AR','AR-C'],
...
const copy = {
  en:{language:'Language',advertisement:'Advertisement',eyebrow:'World meeting time finder',headline:'Find a good time for everyone.',subhead:'Compare cities, choose a day and spot the hours that work—without doing time-zone math.',plannerTitle:'Plan a meeting',addCity:'Add another city',cityPlaceholder:'Search city or country…',add:'Add',meetingDay:'Meeting day',duration:'Duration',findHours:'Find common hours',bestOverlap:'Best overlap',copy:'Copy',share:'Share',adjustTime:'Adjust meeting time',adjustHint:'Drag to explore, or move in 30-minute steps.',workingHours:'Normal working hours',selectedMeeting:'Selected meeting',legendHelp:'Green shows when a city is normally at work; the darker band is your chosen meeting.',simpleTitle:'Simple by design',simpleCopy:'No account required. Dates and times stay in your browser. Holiday checks use country-level public calendar data.',footer:'A friendly world meeting time finder.',selectCity:'Choose a city from the list.',minimumCities:'Keep at least two cities.',holiday:'Public holiday',weekend:'Weekend',noPerfect:'No perfect working-hours overlap. Here is the closest option.',copied:'Copied!'},
...
const state = { language:'en', selected:[cities[0], cities[8]], date:'', duration:60, slot:20, holidays:new Map() };
const els = Object.fromEntries(['language','city-list','city-search','city-options','city-error','add-city','meeting-date','duration','find-hours','results-title','meeting-summary','selected-day','time-slider','timeline','holiday-notices','copy-result','share-result','previous-day','next-day','earlier','later','theme-toggle'].map(id => [id.replaceAll('-','_'), document.getElementById(id)]));
...
function fillCityOptions(){ els.city_options.innerHTML=cities.map(c=>`<option value="${c.name}, ${c.country}"></option>`).join(''); }
function renderCities(){
  const now=new Date();
  els.city_list.innerHTML=state.selected.map((city,index)=>`<div class="city-row"><div class="city-name"><strong>${city.name}</strong><span>${city.country} · ${city.zone}</span></div><div class="city-clock"><strong>${formatTime(now,city.zone)}</strong><span>${index===0?'Your reference city':'Local time now'}</span></div><button class="remove-city" type="button" data-remove="${index}" aria-label="Remove ${city.name}">×</button></div>`).join('');
}
function setLanguage(lang){ state.language=copy[lang]?lang:'en'; document.documentElement.lang=state.language; els.language.value=state.language; document.querySelectorAll('[data-i18n]').forEach(el=>{el.textContent=t(el.dataset.i18n)}); document.querySelectorAll('[data-i18n-placeholder]').forEach(el=>{el.placeholder=t(el.dataset.i18nPlaceholder)}); renderCities(); renderResults(); localStorage.setItem('commonHoursLanguage',state.language); }
...
function calculate(){ const best=findBestSlot(); state.slot=best.slot; renderResults(best.perfect?'':t('noPerfect')); track('overlap_calculated',{cities:state.selected.length,duration:state.duration}); }
...
els.add_city.addEventListener('click',()=>{
  const query=els.city_search.value.trim().toLowerCase();
  const city=cities.find(c=>`${c.name}, ${c.country}`.toLowerCase()===query || c.name.toLowerCase()===query);
  if(!city){ els.city_error.textContent=t('selectCity'); return; }
  if(state.selected.some(c=>c.name===city.name)){ els.city_error.textContent='Already added.'; return; }
  if(state.selected.length>=5){ els.city_error.textContent='Maximum 5 cities.'; return; }
  state.selected.push(city); els.city_search.value=''; els.city_error.textContent=''; renderCities(); calculate(); track('city_added',{city:city.name,country:city.countryCode});
});
els.city_list.addEventListener('click',event=>{ const button=event.target.closest('[data-remove]'); if(!button)return; if(state.selected.length<=2){els.city_error.textContent=t('minimumCities');return;} state.selected.splice(Number(button.dataset.remove),1); renderCities(); calculate(); });
...
state.date=new Date().toISOString().slice(0,10); els.meeting_date.value=state.date; fillCityOptions(); applyTheme(localStorage.getItem('commonHoursTheme') || (matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light')); setLanguage(state.language); calculate();
